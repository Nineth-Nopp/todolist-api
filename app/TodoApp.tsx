...
export default function TodoApp({ initialTodos }: { initialTodos: Todo[] }) {
  ...
  // We don't fetch on mount because we have initialTodos
  // But we could keep it if we want to refresh after some time.
  ...
  const completedCount = todos.length > 0 ? todos.filter((t) => t.completed).length : 0;
  const activeCount = todos.length > 0 ? todos.filter((t) => !t.completed).length : 0;
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <Link href={`/todo/${todo.id}`} style={{ textDecoration: 'none' }}>
                          <p
